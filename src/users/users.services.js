...
const getMyUser = (req, res)=> {
  const id = req.user.id //- <== esto nos trae la informacion del token desencriptado+
...
// -- ruta protegida /me update -- //

const patchMyUser = (req, res)=> {
  const id = req.user.id
  const { firstName, lastName, phone, birthday } = req.body

  usersControllers.updateUser(id, { firstName, lastName, phone, birthday })
  .then(data => {
...
// -- aqui se pueden hacer dos tipos de delete -- //
